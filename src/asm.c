...
inst_t *insts = NULL, *is_tail = NULL;
...
void resolve_names(){
	...
	for(i = insts; i != NULL; i = i->next){
		if(i->type == J){
			...
			if(j == NULL){
				error = 1;
				printf("file:%d:: undefined label %s\n", i->line, i->label);
			}
			else if(j->addr > 0x3FFFFFF){
				error = 1;
				printf("file:%d:: target too far away for jump instruction\n", i->line);
			}else{
				i->imm = j->addr >> 2;
			}
		}
			...
			if(j == NULL){
				...
				if((i->imm & 0x80000000) && (-i->imm <= 0x0000FFFF)){
					...
					error = 1;
					printf("file:%d:: target too far away for branch instruction\n", i->line);
				}
				free(i->label);
				i->label = NULL;
			}
